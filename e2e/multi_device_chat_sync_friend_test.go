...
func TestMultiDeviceChatSyncFriend(t *testing.T) {
	NNodes = 3
	isDebug := true
	...
	marshaled, _ = friend1_12.ID.MarshalText()

	bodyString = fmt.Sprintf(`{"id": "testID", "method": "friend_createMessage", "params": ["%v", %v, []]}`, string(marshaled), string(msg))
	dataCreateMessage1_35 := &friend.BackendCreateMessage{}
	testCore(t1, bodyString, dataCreateMessage1_35, t, isDebug)
	assert.Equal(friend1_12.ID, dataCreateMessage1_35.FriendID)
	assert.Equal(2, dataCreateMessage1_35.NBlock)
	...
	dataGetMessageList1_36_1 := &struct {
		Result []*friend.BackendGetMessage `json:"result"`
	}{}
	testListCore(t1, bodyString, dataGetMessageList1_36_1, t, isDebug)
	assert.Equal(1, len(dataGetMessageList1_36_1.Result))
	message1_36 := dataGetMessageList1_36_1.Result[0]

	dataGetMessageList2_36_1 := &struct {
		Result []*friend.BackendGetMessage `json:"result"`
	}{}
	testListCore(t2, bodyString, dataGetMessageList2_36_1, t, isDebug)
	assert.Equal(1, len(dataGetMessageList2_36_1.Result))

	marshaled, _ = friend2_12.ID.MarshalText()

	bodyString = fmt.Sprintf(`{"id": "testID", "method": "friend_getMessageList", "params": ["%v", "", 0, 2]}`, string(marshaled))

	dataGetMessageList1_36_2 := &struct {
		Result []*friend.BackendGetMessage `json:"result"`
	}{}
	testListCore(t1, bodyString, dataGetMessageList1_36_2, t, isDebug)
	assert.Equal(1, len(dataGetMessageList1_36_2.Result))

	dataGetMessageList2_36_2 := &struct {
		Result []*friend.BackendGetMessage `json:"result"`
	}{}
	testListCore(t2, bodyString, dataGetMessageList2_36_2, t, isDebug)
	assert.Equal(1, len(dataGetMessageList2_36_2.Result))
	...
	friendOplog1_37_0 := dataFriendOplog1_37.Result[0]
	assert.Equal(types.StatusAlive, friendOplog1_37_0.ToStatus())
	assert.Equal(friend.FriendOpTypeCreateFriend, friendOplog1_37_0.Op)
	friendOplog1_37_1 := dataFriendOplog1_37.Result[1]
	assert.Equal(types.StatusAlive, friendOplog1_37_1.ToStatus())
	assert.Equal(friend.FriendOpTypeCreateMessage, friendOplog1_37_1.Op)

	dataFriendOplog2_37 := &struct {
		Result []*friend.FriendOplog `json:"result"`
	}{}
	testListCore(t2, bodyString, dataFriendOplog2_37, t, isDebug)
	assert.Equal(2, len(dataFriendOplog2_37.Result))

	assert.Equal(dataFriendOplog1_37, dataFriendOplog2_37)

	// // 38. get-message-block
	marshaledID2, _ = message1_36.ID.MarshalText()
	marshaledID3, _ = message1_36.BlockID.MarshalText()
