...
func getFriendByMsgCreateTSIter(startingTS types.Timestamp, listOrder pttdb.ListOrder) (iterator.Iterator, error) {
	if startingTS == types.ZeroTimestamp {
		return dbFriend.DB().NewIteratorWithPrefix(nil, DBMessageCreaetTS2Prefix, listOrder)
	}
	...
	// iter
	iter, err := dbFriend.DB().NewIteratorWithPrefix(key, DBMessageCreaetTS2Prefix, listOrder)
	if err != nil {
		return nil, err
	}
